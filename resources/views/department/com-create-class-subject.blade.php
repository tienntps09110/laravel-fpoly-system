            ...
            <div class="box cong-cu-phan-lop p-4">
                ...
                <div class="">
                    <div class="row">
                        ...
                        <div class="col-lg-4 px-5 py-2">
                            <label for=" ">  &nbsp; </label>
                            <button type="button" class="btn btn-block btn-success-neo" id="submit-form" disabled>Xác nhận</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function(){    
        ...
        submitForm.click(()=>{
            $.ajax({
                type:'POST',
                url:'{{ route('create-class-subject-post') }}',
                data:{
                    _token: '{{ csrf_token() }}',
                    class_id:classM.val()?classM.val():null,
                    subject_id:subject.val()?subject.val():null,
                    study_time_id:studyTime.val()?studyTime.val():null,
                    teacher_uuid:teacher?teacher:null,
                    datetime_start:fromDate.val()?fromDate.val():null,
                    datetime_end:toDate.val()?toDate.val():null,
